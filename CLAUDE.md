...
- **Recolour hooks** (set `fill`): `.cc-loco` `.cc-wagon` `.cc-wagon2` `.cc-roof` `.cc-trim` `.cc-brass`. The customizer drives these; user choices must persist. **These names are reserved for rolling stock** — a scene prop that happens to be a wagon or a wheel must not borrow them, or `querySelector('.cc-wagon')` inside a mounted scene can find a coach of the child's own train (see Medora's surrey, which is `.cc-surrey`).
- **A LIVERY is a paint scheme, not a colour.** `<g class="cc-livery" data-livery="flag" display="none">` holds the artwork; `rolling.js`'s `paint()` shows the one whose name matches `colours.livery` and hides the rest, and the customizer offers the swatch only on a vehicle whose drawing actually carries it — it **asks the drawing, never the type**. Every vehicle in the set wears the stars-and-stripes today. `tools/gen-trains.py`'s `flag()` draws it for a rectangular or round-ended body (pass `nose=` the body path's own front fragment for the streamlined shells); the diesel and `steam.svg` are hand-fitted because their shells are irregular. Two rules: the livery goes **over the painted shell and under the fittings** (glass, brass, lamps — a livery appended last blinds the engine), and it **follows the silhouette rather than being clipped to it**, so the scheme needs no `clipPath` and leaves nothing for the id-namespacing in `inline-assets.py` to rewrite.
...
| | State |
|---|---|
| Locations | **55 done — every one of the 50 states, plus DC. The map is complete.** Counted, not guessed: the line below the table says how. Norfolk (VA) is the last one in and the only **causal chain** in the set — a rotary dumper turns a coal wagon upside down, the coal falls out, rides the belt and pours into a ship, and a child can follow one lump the whole way. Its terminal barrier deliberately agrees with the crossing gates: the same rule said twice in one frame. Margate City (NJ): Lucy the Elephant, a six-storey 1882 *building* shaped like an elephant, on an ordinary shore street. Medora (ND): a Northern Pacific depot on the edge of a prairie dog town, with a horse-drawn surrey working the street. Assateague (MD) is the one with a wild animal as the subject — a band of ponies loose on a salt marsh, one drinking from a creek with its reflection under it. **There is no fence in that scene and there must never be one** — it is the whole reason Maryland is that place and not Baltimore. |
...
| `i18n.js` | EN + PL dictionaries, `t()`, `apply()`. Every string a child sees or hears starts here. Also the **`sayAs` phonetic tables** — what the voice is fed when a language's letter rules would mangle a name, via `i18n.sayAs()`. |
...
6. ~~**Do place names get translated?**~~ **RESOLVED, then half of it REVERSED — read both halves.**
   **What is SHOWN follows the data in `world.js`:** a name with a Polish form (`Nowy Jork`, `Nowy Orlean`, `Wielki Kanion`) is displayed in Polish; one without (`Rocky Mountains`, `Seattle`, `San Francisco`) stays English. State names are always English — American proper nouns. **That part stands.**
   **What is SPOKEN no longer follows the name — it follows the player.** The original rule gave an English-named place an English voice, because "Rocky Mountains" with Polish phonetics was thought unrecognisable. That was true of `SpeechSynthesis` and false of recordings: a Polish parent says "Wisconsin Dells" to a Polish child with Polish phonetics. What the rule really produced was the narrator **swapping mid-sentence** — "Następny przystanek" in one voice, "Wisconsin Dells" in another — which is far worse for a three-year-old. So there is now **one narrator per session**, and every place and state is recorded in both sprites.
   `world.spoken(loc)` still returns `{ text, lang }` and `speech.say(text, { lang })` still honours it; nothing in the game passes `lang` any more.
   **Fed English spelling, the Polish voice gets American names wrong** ("Massachusetts" → *masachutes*), so `i18n.js` carries a per-language **`sayAs` respelling table** — 82 entries for Polish, keyed by the text that language DISPLAYS, returning what the voice is fed. `i18n.sayAs(text)` is the lookup; `world.spoken()`, `world.spokenState()` and `map.js` all go through it. **Check a new entry BY EAR** (`tools/voice/venv/bin/piper`) — reading one off the page proves nothing.
   **Know whether you are respelling or translating.** A respelling changes only the voice and lives in `i18n.js`. A different NAME is a translation and belongs in `world.js`'s `say.pl`, which changes the label too (`Rocky Mountains` shows *Góry Skaliste* in Polish). **The trap:** `sayAs` is keyed by the displayed text, so giving a place a Polish name in `world.js` silently orphans its old English-keyed respelling. Move the name, delete the respelling — no tool catches this.
