...
| | State |
|---|---|
| Locations | **54 done, across 49 states + DC** — counted, not guessed: the line below the table says how. **One state left: Virginia**, and it wants re-deciding rather than re-writing, because Lewes has since taken the gantries and the ship that made Newport News distinct. Margate City (NJ) is the newest: Lucy the Elephant, a six-storey 1882 *building* shaped like an elephant, on an ordinary shore street. Medora (ND): a Northern Pacific depot on the edge of a prairie dog town, with a horse-drawn surrey working the street. Assateague (MD) is the one with a wild animal as the subject — a band of ponies loose on a salt marsh, one drinking from a creek with its reflection under it. **There is no fence in that scene and there must never be one** — it is the whole reason Maryland is that place and not Baltimore. |
