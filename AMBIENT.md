...
| **On the carriageway, behind the rails** | `data-over-road="1"` on the mover. Scenery lives in `scenery-back`, which is painted UNDER the road, so anything authored there that crosses the road walks beneath the tarmac — Birmingham's tour party did. The engine moves it into a group of its own between the road and the track. NOT into `cc-cars-far`, which looks like the same slot and is not: leaving a scene empties the two car groups, and scenes are cached and never rebuilt, so anything of the scene's own parked in one would be gone for good. | Birmingham's tour party |
...
| Birmingham | a tour party of four walks the site together, in step · everyone else stands, because a place where everybody moves looks evacuated · the furnace is COLD and stays cold — no pour, no glow |
