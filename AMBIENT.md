...
| **A berth** — lane signal, ramp and queue as ONE sequence | `.cc-ramp-lift` (rotate about its hinge, stood STEEP — about -62 degrees. The handoff asked for -14 and it was wrong here for a reason no handoff could know: a shallow diagonal plank over a road is the exact shape this game uses to mean A GATE ARM IS RAISED, GO, so a child who has learned the crossing reads the shut berth as open, while the traffic in front of it is stopped. Nearly upright it stops being an arm and becomes a linkspan). **The ramp follows the SHIP, not the crossing** — driven by the gate it lifted every time a train came, which reads as the ferry closing up because a train is passing, and a train has nothing to do with a ferry's ramp, `#cc-lane-signal` with `.cc-lamp-red`/`.cc-lamp-green`, `.cc-queuecar` figures that shuffle a few px up their lane on green and drift back on red, and `.cc-ferry` herself, which sails once every couple of minutes and BACKS IN on the way home — a stern-loading ro-ro reverses into its berth so the ramp faces the shore, so astern is the manoeuvre and not a compromise. Cast off and lift the ramp BEFORE she moves: the mooring lines are not part of `#cc-ferry` and would stretch across open water, and a ship leaving with the ramp in her mouth is a crash. While she is away the berth answers the BASCULE'S three questions, and it needs all three: `data-cars` switches itself off, northbound traffic HOLDS at a stop line on the apron instead of driving at an empty berth and fading over open water, and nothing spawns coming the other way, because with no ship there is nowhere a car can have driven off. Miss the second and the cars go swimming; miss the third and they arrive from a ferry that is not there. And the test is **the ramp being DOWN**, not the ship being moored — mooring only starts the ramp on its way and it takes three and a half seconds to arrive, so the ship's own state releases the traffic at a ramp still in the air. Needs TWO clocks — one for the beat before green, one for how long the road has been shut — because sharing one lets the red branch reset it every frame and the ramp never lifts at all. | Lewes |
| **A grazing band** | `.cc-hhead` with `data-lo` / `data-hi` (its own two angles — about +46 is a muzzle in the grass, 0 a resting carriage, -38 alert) and `data-i` for its slot; `.cc-htail` with `data-i`; `.cc-ripple` at the muzzle. **Never two heads at once** — a band that lifts together is a herd startled by something. The cycle is a minute and the moving part ten seconds, so slots cannot overlap. Ripples run ONLY while the muzzle is down. A mirrored REFLECTION copy sharing the same `data-i` tracks the real head for free, which is why the reflection works. | Assateague |
...
| **Tour** — parks, drives off, comes back | `.cc-tour` with `data-bay` (the LEFT EDGE of the parking space), `data-len`, `data-away`, `data-speed`, `data-dwell`, `data-gone`, Never travels backwards: it turns out of sight at the far end and swings round in its own bay. | Glacier's red bus |
...
| **Never borrow a rolling-stock class name** | `.cc-loco` `.cc-wagon` `.cc-wagon2` `.cc-roof` `.cc-trim` `.cc-brass` `.cc-wheel` are the CUSTOMIZER'S hooks, not general-purpose names. Medora's horse-drawn surrey shipped as `class="cc-wagon"` with `class="cc-wheel"` hubs, which is a perfectly reasonable thing to call a wagon and a wheel and is still wrong: `svg.querySelector('.cc-wagon')` at mount time can just as easily find a coach of the child's own train, and a scene prop has no business being addressable by the paint box. It is now `.cc-surrey` / `.cc-surrey-wheel`. Scene props get scene names. | Medora's surrey |
| **Rigging legs that were never rigged** | The canter contract needs `.cc-leg` groups with a `data-pivot` at the joint. Assateague's ponies were drawn as four tidy stacks of paths with a clear attachment point each and simply never wrapped, so its handoff ruled out a walk cycle on the grounds that a bad gait is worse than none. That reasoning is sound and it is ANSWERED rather than overruled: the gait already exists, tuned and shipping in the bluegrass, so wrapping the legs reuses it instead of inventing one. **Keep each hoof with its own leg** — they were appended after both legs, so grouping naively swings a leg and leaves its foot on the ground. | Assateague's band |
...
| **Prairie dogs** — an animal that hides in the ground | `.cc-pdog`, wrapping a drawing that already sits where it belongs; the engine reads the inner group's own translate for its position and only ever writes a vertical offset on the wrapper. It drops fast (0.15s, linear — a prairie dog does not ease into a burrow) and climbs slowly (0.6s, eased), and each animal keeps its OWN clock, so nineteen of them are nineteen independent decisions rather than one wave. **No burrow art is needed** and none should be added: the mound is drawn behind, the animal simply goes down through it, and a 40px drop puts it far enough under to be gone. The four nearest the track are marked as the jumpy ones and go down while a train is coming or the gate is down — the only reason to make the town train-aware at all, and it is worth it, because a child who closes the gate and watches the front row vanish has found something they were not told about. | Medora's dog town |
| **Going in one way and coming out another** | `.cc-door` and `.cc-howdah`, two groups the engine fades in strict alternation. **The empty beat between them is the whole trick and must never be tuned away**: a child who sees figures at a door in the elephant's leg, then sees NOTHING, then sees figures appear at a railing sixty feet up works out on their own that she is hollow and there are people climbing about inside. Let the two overlap and nothing is learned — it is two sets of dots lit at different times. Both groups' opacities come off ONE clock so they cannot be lit together by a rounding accident (measured: 1314 samples, zero frames with both visible). Opacity only — do not translate anybody up the inside of the leg, because you cannot see inside her. It is the prairie dogs' appear-and-vanish verb pointed the other way. | Margate's Lucy |
| **A kite on a LOOP, not on a line** | `.cc-kite` with no `data-anchor`, plus a small closed `#kite-path`. Los Angeles's kite is tethered — it swings on a string from a named hand and the string is redrawn to follow it; Margate's has nobody holding it and what it does is WANDER. Forty seconds a lap: quicker and it is a fly rather than something hanging on a hundred feet of string. It tilts into the turn from its own heading. The engine reads the drawn position as an OFFSET from the path's start rather than snapping the art onto the curve. | Margate |
...
| **A single-track side road** | A 4th token `single` on `data-exit`. The other two shapes share one lane by running BOTH STREAMS THE SAME WAY (`east`); this one shares it by **taking turns**, because Margate's beach-lot aisle is 35px deep — a car at that depth is 35px tall, so exactly one lane exists — and its inbound traffic has to arrive from the EAST, which `east` cannot do. One token, one car at a time. **Alternate; do not prioritise.** Both priorities were tried and both starve: first-come hands it to the inbound queue, which is refilled the instant it moves, and nothing ever drives out (five in, none out in a minute); giving it to the one waiting in plain sight loses to outbound traffic, which is continuous (seven out, none in). Alternating gives three and three. The outbound car waits ON THE CARRIAGEWAY at `AISLE_WAIT`, south of the crossing — the junction is at y=441 and the keep-clear band starts at 438, so a car pausing at the junction mouth is a car stopped on the rails. And the side road needed the spawn guard the carriageway always had: six inbound cars piled up off frame and ate the whole eight-car budget while the scene stood empty. | Margate |
| **Where the road goes next** | `.cc-road-exit` with `data-exit="farEdgeY,nearEdgeY,junctionX"`, plus an optional 4th token `east`. Without it the side road runs two opposed lanes (Rim Drive). With it BOTH streams run east in one lane — off the carriageway and away, and in from the far west then right onto the carriageway — which is the only way a narrow street can carry two directions. `nearEdgeY` must sit ON the tarmac, or the car is deleted at the road's end before it can turn. **A car joining the carriageway must be able to be stopped by the gate.** `nearEdgeY` is where a joining car is put down, and at Crater Lake that is y=432 — eleven pixels UNDER the far arm and thirty-six under `STOP_FAR`. The gate's test is a position test standing in for "has not passed the gate yet", so every car that turned down off Rim Drive read as already-past and drove through a closed crossing, four in twenty-five seconds. Two fixes, and both were needed: a backstop in `updateCars` that lets NOTHING enter the track band while the crossing is shut (only for a car that has not reached it — one already on the rails has to clear them), and a hold at the junction, because the backstop alone stops the car six pixels below the junction and there is no road to queue on between there and the rails. It waits on the side road, where there is room and where a driver would. | Crater Lake, Sun Valley, Indianapolis |
