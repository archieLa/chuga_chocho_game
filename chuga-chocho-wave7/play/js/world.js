...
(function (CC) {
  ...
  const LOCATIONS = [
    ...
    { id:'newrivergorge', state:'West Virginia', city:'New River Gorge',
      say:{ en:'New River Gorge', pl:'Wąwóz New River' },
      scene:'new-river-gorge',
      // Viewpoint: the canyon floor at Fayette Station. The road comes down the wall,
      // crosses the railroad and runs into the Tunney Hunsaker truss; the great arch is
      // 876 feet overhead. The set's first gorge and its first landmark ABOVE the horizon.
      scenery:{ theme:'appalachian-gorge', features:['steelarch','rapids','trussbridge','rhododendron'] },
      trainPreset:{ engine:'diesel' } },

    { id:'mountrushmore', state:'South Dakota', city:'Mount Rushmore',
      say:{ en:'Mount Rushmore', pl:'Mount Rushmore' },
      scene:'mount-rushmore',
      // Viewpoint: Keystone, the town at the foot of the memorial and the terminus of the
      // Black Hills Central — the 1880 Train, whose railroad hauled the equipment that
      // carved the mountain behind it. Hence the steam preset.
      scenery:{ theme:'black-hills', features:['carving','granite','ponderosa','talus'] },
      trainPreset:{ engine:'steam' } },

    { id:'vicksburg', state:'Mississippi', city:'Vicksburg',
      say:{ en:'Vicksburg', pl:'Vicksburg' },
      scene:'vicksburg',
      // Viewpoint: the waterfront below the bluff. The set's first Mississippi and its
      // first river freight — a towboat pushing fifteen barges. The road ends at a
      // floodwall gate, which is a thing no other scene has.
      scenery:{ theme:'lower-mississippi', features:['towboat','barges','bluff','floodwall'] },
      trainPreset:{ engine:'diesel' } },

    { id:'newport', state:'Rhode Island', city:'Newport',
      say:{ en:'Newport', pl:'Newport' },
      scene:'newport',
      // Viewpoint: Thames Street looking out over the harbour. The set's first SAIL, and
      // the opposite kind of water from Stonington's working lobster harbour.
      scenery:{ theme:'new-england-harbour', features:['sailboats','pellbridge','wharves','cobbles'] },
      trainPreset:{ engine:'steam' } },
