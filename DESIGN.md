...
- **Engine types (6 powered):** steam, diesel-electric, high-speed electric, commuter EMU, streetcar, cable car — also the *history* of trains, narratable as a lesson.
...
**Built:** all **14** vehicles (6 powered + 8 wagons) + `manifest.json`, generated by `tools/gen-trains.py` (steam is hand-authored). `tools/build-gallery.py` builds an inspection page. The child's consist is **one locomotive + exactly three wagons**; `play/js/trains.js` is the data layer and `play/js/customizer.js` the cycling UI.
